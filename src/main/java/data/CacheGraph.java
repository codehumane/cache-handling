...
public class CacheGraph<T> implements Serializable {
    ...
    public boolean addEdge(T from, T to) {
        ...
        boolean linkedTo = link(new CacheGraphVertex<>(from), new CacheGraphVertex<>(to));
        boolean linkedFrom = link(new CacheGraphVertex<>(to), new CacheGraphVertex<>(from));
    ...
    public boolean removeEdge(T from, T to) {
        ...
        boolean unlinked = unlink(new CacheGraphVertex<>(from), new CacheGraphVertex<>(to));
        unlink(new CacheGraphVertex<>(to), new CacheGraphVertex<>(from));
        return unlinked;
    }
    ...
    public List<T> getAdjacentVertexes(T from) {
        ...
        Cache.ValueWrapper valueWrapper = cache.get(new CacheGraphVertex<>(from));
        if (valueWrapper == null) {
            return Lists.newArrayList();
        }
